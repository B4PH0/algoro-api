...
export async function SignupController(req: Request, res: Response, next: NextFunction) {
    ...
    if (!name) return res.status(422).json({ msg: 'Undefined name, please provide one.'});
    if (!email) return res.status(422).json({ msg: 'Undefined email, please provide one'});
    if (!password_hash) return res.status(422).json({ msg: 'Undefined password, please provide one.'});
